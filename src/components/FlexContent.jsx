      ...
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill transitions-theme ${ifExists ? 'h-100 lg:h-120 md:h-70 sm:h-60 xsm:h-50 rotate-6 hover:-rotate-12' : 'h-90 lg:h-110 md:h-80 sm:h-70 xsm:h-60 rotate-[19deg] hover:rotate-12'}`}
